...
export default injectIntl(
  class extends React.PureComponent {
    ...
    deleteTimer = () => {
      ...
      Modal.confirm(
        formatMessage({ id: 'internal.SDKTimer.confirmDelete' }),
        () => {
    ...
    render() {
      ...
      return (
        <Page saveBottom>
          <NavBar
            title={formatMessage({ id: 'internal.SDKTimer.timingSet' })}
            right={rightBtn}
          />
          <TimePicker
            onChange={this.timeUpdate}
            defaultValue={timer.time || moment().add(1, 'minutes')}
          />

          <div className={style.listBox + ' ' + style.mt20}>
            <SettingItem
              title={formatMessage({ id: 'internal.SDKTimer.repeatSetting' })}
              content={repeatDesc}
              onClick={() => history.push('/timer/repeat')}
            />
            {false && (
              <SettingItem
                title={formatMessage({ id: 'internal.SDKTimer.date' })}
                content={dateDesc}
                onClick={() => history.push('/timer/date')}
              />
            )}
            <SettingItem
              onClick={() => history.push('/timer/command')}
              title={formatMessage({ id: 'internal.SDKTimer.operation' })}
              content={this.commandDesc()}
            />
          </div>
          {timer.index != null && (
            <div
              className={style.saveBox + ' ' + style.delBox}
              onClick={this.deleteTimer}
            >
              <FormattedMessage id="internal.SDKTimer.delete" />
            </div>
          )}
          <BottomButton
            text={formatMessage({ id: 'internal.SDKTimer.timerSave' })}
            onClick={this.save}
            enable={!isEmpty(timer.status)}
          />
        </Page>
      );
    }
