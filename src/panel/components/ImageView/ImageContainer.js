...
class ImageContainer extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
        ...
        top: 0,
        isLoaded: false,
        src: props.src.indexOf('?') > -1 ? `${props.src}&v=${new Date().getTime()}` : `${props.src}?v=${new Date().getTime()}`
    }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { screenHeight, screenWidth } = nextProps;
    if(nextProps.zIndex !== zIndex){
        if (this.animationID) {
