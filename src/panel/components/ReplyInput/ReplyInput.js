...
import { selectPicture, uploadFileByApp } from './../../sdk-util.js';
...
class ReplyInput extends React.PureComponent {
    ...
    handleChangeDesc(e) {
        ...
        let len1 = v.match(/[\u4E00-\u9FA5]/g) ? v.match(/[\u4E00-\u9FA5]/g).length : 0
        let len2 = v.length;
        let len = len1 + len2;
        if(len <= 400){
            this.setState({
                ...
                this.setState({
                    ...
                    uploadFileByApp(res.path).then((file)=>{
                        ...
                        if(file && file !== null && file.status === 0){
                            ...
                            newFiles2[index].status = newFiles2[index].status === 'loading' ? 'success' : newFiles2[index].status
                            this.setState({
                                files: [...newFiles2]
                            });
                        }else{
                            let newFiles2 = [...files];
                            newFiles2[index].status = newFiles2[index].status === 'loading' ? 'error' : newFiles2[index].status
                            this.setState({
                                files: [...newFiles2]
                            });
                        }
            ...
            this.setState({
                ...
                uploadFileByApp(files[i].path).then((file)=>{
                    ...
                    if(file && file !== null && file.status === 0){   
                        ...
                        newImages2[i].status = newImages2[i].status === 'loading' ? 'success' : newImages2[i].status
                        this.setState({
                            files: [...newImages2]
                        });
                    }else{
                        newImages2[i].status = newImages2[i].status === 'loading' ? 'error' : newImages2[i].status
                        this.setState({
                            files: [...newImages2]
                        });
                    }
    ...
    render(){
        ...
        const { focus , message, files, replyName, isOpenView, viewIndex, viewFiles} = this.state;
        let showLen = 0;
        files.forEach((_e)=>{
            if(_e.status !== 'delete'){
                showLen+=1
        ...
        return(
           focus ? 
            <div className={classNames("replyBoxInputWrap", {"isIphoneX": Device.isIphoneX, "disbled": !(message2.length > 0 || showLen > 0)})}>
                <NavBar 
                    title={intl.formatMessage({id: "comment"})} 
                    right={{text: intl.formatMessage({id: "send"}), "handler":  this.handleSendBtn.bind(this)}} 
                    left={"close"}
                    onBack={this.handlePageBack.bind(this)}/>
                <div className="replyBoxInput" ref={(ref)=>{this.inputDom = ref}}>
                    <div className="left">
                        <textarea 
                            ref={(input) => { this.rePlyText = input; }}
                            onChange={this.handleChangeDesc.bind(this)}
                            placeholder={ replyName ? intl.formatMessage({id: "replyToName"}, {name: replyName}) 
                         ...
                         <ul className="imgList">
                            {
                                files.length > 0 ? files.map((item, _i)=>{
                                    return item.status !== 'delete' ? <li key={_i} className="imgItem bgImgItem" >
                                                {
                                                    item.status === 'success' ? <div 
                                                        className="uploadImgWrap"
                                                        style={{"backgroundImage": `url(${item.src}`}} 
                                                        onClick={this.handleViewImage.bind(this, _i)} ></div> 
                                                    : null
                                                }
                                                {
                                                    item.status === 'error' ? <div 
                                                        ...
                                                        <div className="del" onClick={(e)=>{this.handleDeleteImg(e, _i)}}></div>
                                                        </div>  
                                                    : null
                                                }
                                                {
                                                    item.status === 'loading' ? <div 
                                                        className="uploadImgWrap uploadLoading"
                                                        >
                            ...
                            {
                                showLen < 6 ? <li className="imgItem addImg" onClick={this.handleCamera.bind(this)}>
                                </li> : null
                            }
                         </ul>  
                    </div>
               </div>
                {
                    <WxImageViewer 
                        ...
                        urls={viewFiles} 
                        index={viewIndex}
                        pointer={<div className={classNames("viewPointer", {"isIphoneX": Device.isIphoneX})}>{`${viewIndex+1}/${viewFiles.length}`}</div>}
                        changeIndex={this.changeIndex.bind(this)}
                    />
                } 
           </div>: showReply ? <div className= {classNames("replyBox")} onClick={this.handleClickFocus.bind(this)}>
                <div className="tomit placeBox">{intl.formatMessage({id: "postYourQuestion"})}</div>
            </div> : null
        );
    }
}
