...
program
  ...
  .option(
    '-k, --sourcemap',
    '保留sourcemaps(DNA平台的prod包默认不生成sourcemaps)'
  )
...
checkBrowsers(paths.appPath, isInteractive)
  .then(() => {
    ...
    if (iotPlatform === 'dna' && appPackage.supportCustomScene) {
      console.error(appPackage.supportCustomScene);
      checkStringsFiles();
    }
  ...
  .then(
    ({ stats, previousFileSizes, warnings }) => {
      ...
      if (iotPlatform === 'dna') {
        if (appPackage.supportCustomScene) {
...
function checkStringsFiles() {
  ...
  const profileIntfs = execJsFile(path.join(paths.appPublic, 'profile.js'))
    .PROFILE.suids[0].intfs;
  const stringsIntfsArr = stringsFiles.map(
    (file) => execJsFile(path.join(paths.appPublic, file)).STRINGS.intfs
  );
...
function copyStringsFileToZhCn() {
  ...
  fs.copySync(
    path.join(paths.appPublic, stringsFiles[0]),
    path.join(paths.appBuild, 'zh-cn', 'strings.js')
  );
}
